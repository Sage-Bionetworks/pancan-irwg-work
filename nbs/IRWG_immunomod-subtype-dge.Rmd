...
immunomod_subtype_sample_df <- mrna_subtype_mclust_shared_ids %>% 
    ...
    left_join(sample_qual_df %>% 
                  select(aliquot_barcode, disease = `cancer type`) %>% 
                  distinct(), 
              by = c("id_mrna" = "aliquot_barcode")) %>%
    select(vial_id, Center, platform, Adjustment, ClusterModel1, disease) %>%
...
mrna_immunomod_pca_df %>% 
    spread(PC, value) %>% 
    ggplot(aes(x = `1`, y = `2`, colour = ClusterModel1)) +
...
dge_df <- mrna_immunomod_df %>% 
    mutate(log_x = log(x + 1)) %>% 
    group_by(gene_name) %>% 
    # do(tidy(anova(lm(log_x ~ 0 + ClusterModel1 + Center, .)))) %>%
...
```{r}
mrna_immunomod_df %>% 
    filter(gene_name %in% c("EDNRB", "CXCL10", "ICOS", 
                            "CCL5", "TNFRSF18", "CTLA4")) %>%
    # filter(gene_name %in% c("HMGB1", "CD276", "HLA-C", 
    #                         "HLA-A", "HLA-B", "VEGFB")) %>%
    mutate(log_x = log(x + 1)) %>%
    ggplot(aes(y = log_x, x = ClusterModel1, colour = ClusterModel1)) +
