...
```{r}
# im_subtype_logexpr_df <- im_logexpr_df %>% 
...
#     ))
# readr::write_csv(im_subtype_logexpr_df, "../data/intermediate/subtype_immunomodulator_expr.csv")
```

```{r}
im_logexpr_df %>% 
    select(Subtype_Immune_Model_Based, gene_name, log_x) %>% 
    group_by(Subtype_Immune_Model_Based, gene_name) %>% 
    summarise(frac_na = sum(is.na(log_x)) / length(log_x)) %>% 
    ungroup() %>% 
...
```{r}
# immunomod_subytype_expr_mat <- immunomod_subtype_expr_df %>%
#     dplyr::select(gene_name, Subtype_Immune_Model_Based, `Super Category`, x_med) %>% 
...
# gene_annot_df <- tibble::tibble(label = rownames(immunomod_subytype_expr_mat)) %>% 
...
# )
# pheatmap::pheatmap(immunomod_subytype_expr_mat,
...
im_arrow_df <- im_factor_scores$factors$centroids %>% 
    ...
    mutate(factor = str_extract(arrow, 
                                str_c(names(im_factor_df), 
                                      collapse = "|")),
           arrow = str_replace(arrow, 
                               str_c(names(im_factor_df), 
                                     collapse = "|"), 
                               "")) %>% 
    left_join(im_factor_scores$factors$r %>% 
                  as.data.frame() %>% 
                  rownames_to_column() %>%
                  set_names(c("factor", "r2")),
              by = "factor") %>% 
    left_join(im_factor_scores$factors$pvals %>% 
                  as.data.frame() %>% 
                  rownames_to_column() %>%
                  set_names(c("factor", "pval")),
              by = "factor") %>% 
    bind_rows(im_factor_scores$vectors$arrows %>% 
    as.data.frame() %>% 
    rownames_to_column("arrow") %>% 
    mutate(factor = str_extract(arrow, 
                                str_c(names(immunomod_factor_df), 
                                      collapse = "|"))) %>% 
    left_join(im_factor_scores$vectors$r %>% 
                  as.data.frame() %>% 
                  rownames_to_column() %>%
                  set_names(c("factor", "r2")),
              by = "factor") %>% 
    mutate(pval = im_factor_scores$vectors$pvals)) %>% 
...
p <- im_logexpr_df %>% 
    ...
    ggbeeswarm::geom_quasirandom(
        ggplot2::aes(fill = Subtype_Immune_Model_Based, colour = Subtype_Immune_Model_Based), 
        size = 0.3, alpha = 0.5,
        shape = 21, colour = "slategray", stroke = 0.1
    ) +
...
ggsave("../figures/im_subtype_expr_dists.pdf", p,
       width = 6, height = 3.5, units = "cm", dpi = 300)
```
