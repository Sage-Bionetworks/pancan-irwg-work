...
# only keep samples with matched vial AND portion
mirna_leuk_frac_shared_ids <- mirna_leuk_frac_shared_ids %>% 
    filter(str_extract(id_mirna, "([:alnum:]+\\-){4}[0-9]+") 
           == str_extract(id_leuk_frac, "([:alnum:]+\\-){4}[0-9]+"))
```
...
force_read <- TRUE
if (!file.exists(leuk_frac_corr_file) | force_read) {
...
if (!file.exists(mirna_leuk_frac_corr_file) | force_compute) {
    ...
    d_list <- mirna_sample_pt_df %>%
        filter((!Disease %in% c("LAML", "THCA", "DLBC")),
               Disease %in% leuk_frac_corr_df$disease) %>% 
        distinct(Disease) %>% 
...
ciber_frac_ids <- ciber_frac_df %>% 
    select(id = SampleID) %>%
    mutate(vial_id = str_replace_all(id, "\\.", "\\-")) %>% 
    arrange()
...
force_read <- TRUE
if (!file.exists(ciber_frac_corr_file) | force_read) {
    ciber_frac_corr_df <- mirna_ciber_frac_shared_ids %>% 
        left_join(ciber_frac_df, by = c("id_ciber_frac" = "SampleID"))
...
force_read <- TRUE
if (!file.exists(mut_load_corr_file) | force_read) {
...
mirna_mrna_shared_ids <- mirna_mrna_shared_ids %>% 
    filter(str_extract(id, "([:alnum:]+\\-){4}[0-9]+") 
           == str_extract(SampleID, "([:alnum:]+\\-){4}[0-9]+"))
```
...
force_read <- TRUE
if (!file.exists(mrna_corr_file) | force_read) {
    # load normalized, batch-corrected expression data
