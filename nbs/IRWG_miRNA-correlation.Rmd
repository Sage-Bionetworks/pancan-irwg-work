...
mirna_sig_corr_df %>% 
    ...
    group_by(disease, mirna, group) %>%
    summarise(estimate = mean(estimate)) %>%
    ungroup() %>% 
    mutate(direction = ifelse(estimate > 0, "positive", "negative")) %>%
    group_by(mirna, group, direction) %>%
    summarise(diseases = n_distinct(disease)) %>%
    group_by(mirna) %>%
    mutate(nz_diseases = sum(diseases > 0)) %>%
    ungroup() %>%
    filter(nz_diseases > 10) %>% 
    arrange(desc(nz_diseases)) %>%
    mutate(mirna = fct_inorder(mirna)) %>%
    I %>%
    ggplot(aes(y = group, x = mirna)) +
    geom_tile(aes(fill = diseases)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r}
mirna_causal_df %>% 
    filter(`miRNA Name` %in% common_mirnas) %>% 
    group_by(`miRNA Name`, `Tumor Type`) %>%
    tally() %>% 
    group_by(`miRNA Name`) %>%
    summarize(sum(n > 0)) %>% 
    ungroup()
