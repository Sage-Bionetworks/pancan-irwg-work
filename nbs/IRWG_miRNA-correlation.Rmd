...
mirna_all_corr_df %>% 
    filter(p.adjust < 0.05, abs(estimate) > 0.5) %>% 
    distinct(mirna)
    # group_by(mirna) %>% 
    # filter(n_distinct(disease) > 10) %>% 
    # ungroup() %>% 
    # # filter(disease %in% c("ACC", "CESC", "DLBC", "ESCA", "SKCM")) %>%
    # filter(correlate_type != "mRNA expression") %>%
    I
    # ggplot(aes(x = correlate, y = disease)) +
    # geom_point(aes(colour = disease)) +
    ...
    # facet_grid(mirna ~ correlate_type, space = "free", scales = "free") %>% 
```
