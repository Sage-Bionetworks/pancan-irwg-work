...
if (!file.exists(mirna_mrna_corr_file)) {
...
mirna_mrna_corr_df %>% 
    filter(disease == "BRCA") %>% 
    group_by(mirna) %>% 
    filter(any(p.adjust < 0.01), any(abs(estimate) > 0.5)) %>%
    ungroup() %>% 
    left_join(gene_correlate_df, by = c("mrna" = "HGNC Symbol")) %>% 
    group_by(mirna, `Gene Family`) %>% 
    summarize(avg_correlation = mean(estimate)) %>% 
    ggplot(aes(x = `Gene Family`, y = mirna)) +
    geom_tile(aes(fill = avg_correlation)) +
