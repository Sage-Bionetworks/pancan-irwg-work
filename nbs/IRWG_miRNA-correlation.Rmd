...
Note: only include "primary" tumor samples for now
...
mrna_ids <- mrna_sample_df %>% 
    select(SampleID) %>%
    mutate(tumor = str_extract(SampleID, "([:alnum:]+\\-){4}[0-9]+[A-Z]")) %>% 
    arrange()

mirna_ids <- mirna_sample_df %>%
    select(id) %>%
    mutate(tumor = str_extract(id, "([:alnum:]+\\-){4}[0-9]+[A-Z]")) %>% 
    arrange()

tumor_ids <- inner_join(mirna_ids, mrna_ids, by = "tumor")
```
