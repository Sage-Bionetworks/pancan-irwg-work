...
# only keep samples with matched vial ID AND portion number
mirna_subtype_mclust_shared_ids <- inner_join(
    mirna_ids, subtype_mclust_ids,
    by = "vial_id", suffix = c("_mirna", "_subtype_mclust")
    ) %>% 
    distinct() %>% 
    filter((str_extract(id_mirna, portion_id_minus_analyte_regex) 
...
```{r}
subtype_mclust_corr_file <- "../data/intermediate/subtype_mclust_correlates_for_mirna.feather"
force_format <- FALSE
if (!file.exists(subtype_mclust_corr_file) | force_format) {
    subtype_mclust_corr_df <- mirna_subtype_mclust_shared_ids %>%
...
## Recurring immune-associated miRNAs
...
## All supported miRNA-immunomodulator associations

```{r, fig.width=4, fig.height=8, message=FALSE, warning=FALSE}
plot_colors <- tcga_colors %>% 
    filter(Disease %in% filter(mirna_support_df, 
                               mirbase_support & immune_support)$disease)

p <- mirna_support_df %>%
    filter(mirbase_support & immune_support) %>% 
    group_by(label) %>%
    mutate(num_diseases_gene = n_distinct(disease)) %>%
    group_by(mirna) %>%
    mutate(num_diseases_mirna = n_distinct(disease)) %>%
    ungroup() %>%
    filter(num_diseases_gene > 10,
           num_diseases_mirna > 10) %>%
    mutate(end3p = str_detect(mirna_short, "3p$")) %>% 
    # filter(mirna_short_ambiguous == "miR-182") %>%
    mutate(disease = factor(disease, levels = plot_colors$Disease)) %>%
    ggplot(aes(x = 1, y = mirna_short_ambiguous)) +
    geom_dotplot(aes(fill = disease, colour = end3p), dotsize = 12,
                 binaxis = "y", stackgroups = TRUE, positions = "all") +
    # geom_tile(aes(fill = disease)) +
    scale_colour_manual(values = c("black", "#CCCCCC")) +
    scale_fill_manual(values = plot_colors$Color) +
    xlab("") +
    ylab("") +
    guides(colour = FALSE,
    ...
    facet_grid(group + label ~ ., scales = "free_y", space = "free_y") %>%
    I
p
# scales::show_col(viridis(8))
# ggsave("../figures/common_mir_immunomod_corr.png", p,
#        width = 7.5, height = 12, units = "cm", dpi = 300, scale = 1.8)
```
...
## miRNA and immune subtypes

```{r}
# subtype_mclust_corr_df 
mirna_cluster_df <- mirna_corr_df %>% 
    set_names(str_replace(names(.), "(\\-[:alnum:]+){3}$", "")) %>%
    select(one_of(c("Genes", mirna_subtype_mclust_shared_ids$vial_id))) %>%
    dplyr::rename(mirna = Genes) %>%
    gather(sample, x, -mirna) %>% 
    filter(!is.na(x)) %>% 
    left_join(select(mirna_sample_pt_df, Disease, vial_id),
              by = c("sample" = "vial_id")) %>% 
    left_join(subtype_mclust_corr_df, by = c("sample" = "vial_id")) %>% 
    arrange(ClusterModel1, Disease)
```

```{r}
head(mirna_cluster_df)
```

```{r}
mirna_cluster_df %>% 
    group_by(mirna, Disease, ClusterModel1) %>% 
    summarize(avg_expression = mean(x)) %>% 
    ggplot(aes(x = Disease, y = mirna)) +
    geom_tile(aes(fill = log10(avg_expression + 1))) +
    facet_grid(~ ClusterModel1, scales = "free_x", space = "free_x")
```


```{r}
mirna_cluster_mat <- mirna_cluster_df %>%
    filter(!(Disease %in% c("DLBC", "THYM", "LAML"))) %>%
    select(mirna, sample, x) %>% 
...
col_colors <- tibble(sample = colnames(mirna_cluster_mat)) %>% 
    left_join(select(mirna_cluster_df, 
                     sample, Disease, ClusterModel1) %>% 
                  distinct(),
              by = "sample") %>% 
    left_join(tcga_colors, by = "Disease")

gplots::heatmap.2(mirna_cluster_mat, Colv = FALSE,
